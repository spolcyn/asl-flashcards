...
def render_docs() -> None:
    doc_path: Path = Path(__file__).parent.parent / "docs" / "docs.md"
    with open(doc_path, mode="r") as doc_file:
        doc_body: str = doc_file.read()

    st.markdown(doc_body, unsafe_allow_html=True)


def render_app() -> None:
    vocab_timing_csv: Optional[BytesIO] = st.file_uploader(
        "CSV file with Vocab and Timing info", type=["csv"]
    )
    segment_string: Optional[str] = None
    vocab_timing_df: Optional[pd.DataFrame] = None
    if vocab_timing_csv:
    ...
    finished_computation: bool = False
    split_video_dir: Optional[str] = None
    video_split_dependencies: list[Optional[bool]] = [vocab_timing_df is not None, segment_string]
    if all(video_split_dependencies):
        video_file: Optional[BytesIO] = st.file_uploader("Video with words signed")
        if video_file:
            video_data: bytes = video_file.getvalue()
    ...
    zip_videos_dependencies: list[Optional[bool]] = [finished_computation, split_video_dir]
    if all(zip_videos_dependencies):
        zip_output_path: str = zip_dir(split_video_dir)  # type: ignore
    ...
    anki_import_dependencies: list[Optional[bool]] = [words is not None, video_paths is not None]
    if all(anki_import_dependencies):
        ...
        done_with_tags: bool = st.button("Finish adding tags and generate flashcard deck")
        if not done_with_tags:
            return

        card_tags: list[str] = [tags for _ in range(len(words))]
        ...
        anki_import_csv: bytes = bytes(
            anki_import_df.to_csv(header=False, index=False),
            encoding="utf-8",
        )
